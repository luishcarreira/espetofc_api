...
@router.post("/caixas/", response_model=schemas_caixa.Caixa)
...
@router.get("/caixas/{caixa_id}", response_model=schemas_caixa.Caixa)
...
@router.get("/caixas/", response_model=List[schemas_caixa.Caixa])
def read_caixas(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    caixas = crud_caixa.get_caixas(db, skip=skip, limit=limit)
    return caixas

@router.put("/caixas/{caixa_id}", response_model=schemas_caixa.Caixa)
...
@router.delete("/caixas/{caixa_id}", response_model=bool)
